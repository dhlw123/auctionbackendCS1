...
/**
 * Bộ xử lý ngoại lệ toàn cục (Global Exception Handler). Tự động bắt các loại ngoại lệ xảy ra trong
 * Controller và định dạng lại dữ liệu phản hồi trả về cho Client.
 */
...
public class GlobalExceptionHandler {
  ...
  /**
   * Xử lý lỗi xác thực dữ liệu đầu vào (Validation Errors) từ @Valid. Trả về mã lỗi HTTP 400 (Bad
   * Request) cùng với danh sách các trường bị lỗi và thông điệp tương ứng.
   */
  ...
  /**
   * Xử lý ngoại lệ nghiệp vụ tự định nghĩa BaseException. Trả về mã lỗi HTTP 400 (Bad Request) kèm
   * theo đối tượng BaseResponse chứa thông điệp lỗi.
   */
  ...
  /**
   * Xử lý lỗi khi không tìm thấy thực thể trong cơ sở dữ liệu (EntityNotFoundException). Trả về mã
  ...
  /**
   * Xử lý lỗi khi mã token JWT hết hạn (JwtExpiredException). Trả về mã HTTP 498 (Token
   * expired/invalid).
   */
