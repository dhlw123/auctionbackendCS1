...
/**
 * Cấu hình bảo mật hệ thống Spring Security. Thiết lập các quy tắc phân quyền (Authorization), mã
 * hóa mật khẩu, cấu hình CORS và cơ chế JWT.
 */
@Configuration
...
public class SecurityConfig {
  ...
  /**
   * Cấu hình CORS (Cross-Origin Resource Sharing) cho phép frontend (React chạy trên cổng 5173 hoặc
   * 3000) có thể gọi API từ backend này.
   */
  @Bean
  ...
  /**
   * Cấu hình bộ lọc bảo mật chính (SecurityFilterChain) định nghĩa cách kiểm soát các request HTTP
   * gửi đến.
   */
  @Bean
            ...
            s ->
                s.sessionCreationPolicy(
                    org.springframework.security.config.http.SessionCreationPolicy
                        .STATELESS // Không lưu session phía server
                    ))
        .authorizeHttpRequests(
            auth ->
                auth
                    ...
                    .requestMatchers(HttpMethod.GET, "/items/**", "/item/status/**")
                    .permitAll()
