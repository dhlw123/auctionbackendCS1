...
public interface BidRepository extends JpaRepository<Bid, Long> {
  ...
  /**
   * Lấy phân trang lịch sử đặt cược của một sản phẩm cụ thể. Sử dụng JPQL Custom Query kết hợp JOIN
   * FETCH b.item để tránh lỗi N+1 query.
   *
  ...
  /**
   * Truy vấn JPQL tìm các lượt đấu giá chiến thắng của người dùng, kèm eager-load Item qua JOIN
   * FETCH để tránh N+1 query khi truy cập {@code bid.getItem()}. Một người thắng cuộc khi: - Họ đặt
   * giá cho sản phẩm ({@code b.user.username = :username}) - Họ giữ giá cao nhất hiện tại ở bảng
   * trạng thái ({@code s.highestBidUser = :username}) - Thời gian đấu giá của sản phẩm đó đã trôi
   * qua ({@code s.endTime < :now})
   *
   ...
   */
  @Query(
      "SELECT b FROM Bid b JOIN FETCH b.item JOIN ItemStatus s ON b.item = s.item WHERE b.user.username = :username AND s.highestBidUser = :username AND s.endTime < :now")
